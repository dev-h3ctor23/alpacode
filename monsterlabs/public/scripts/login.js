...
document.getElementById("loginForm").addEventListener("submit", function(event) {
    ...
    fetch('/alpacode/monsterlabs/mvc/controllers/iniciar-sesion.php', {
        method: 'POST',
        body: formData
    })
    ...
    .then(text => {
        ...
        try {
            ...
            if (data.status === "success") {
                window.location.href = `/alpacode/monsterlabs/mvc/views/${data.user_type}.html`;
            } else {
                alert(data.message);
            }
        } catch (error) {
            console.error("Error al procesar JSON. Respuesta recibida:", text);
        }
    })
